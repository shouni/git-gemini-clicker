...
**Git Gemini Clicker** は、**Google Gemini の強力なAI**を活用して、**ローカルGitリポジトリの差分（diff）に基づいたコードレビューを自動化**するコマンドラインツールです。インストール後の実行コマンドは **`ggrc`** です。
...
* **最高の堅牢性**: **指数バックオフ付きのリトライ・遅延メカニズム**を実装し、レートリミットやサーバーエラーに耐性を持たせることで、堅牢な設計思想を完全に継承しています。
...
| **Git 操作** | **Git コマンド (subprocess)** | ローカルGitリポジトリ操作（クローン、ブランチ切り替え、**3点比較による差分取得**）を直接実行 |
